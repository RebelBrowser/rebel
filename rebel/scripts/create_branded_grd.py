...
class Browser(object):
    ...
    def __init__(self, name, company, schema):
        self.name = name
...
def branded_string(browser, message, force_google_substitution=False):
    """
...
def branded_element(browser, element, force_google_substitution=False):
    ...
    (element.text, google) = branded_string(
        browser, element.text, force_google_substitution)
    did_google_substitution = did_google_substitution or google

    (element.tail, google) = branded_string(
        browser, element.tail, force_google_substitution)
    did_google_substitution = did_google_substitution or google
    ...
    for placeholder in element.findall('ph'):
        ...
        (placeholder.tail, google) = branded_string(
            browser, placeholder.tail, force_google_substitution)
        did_google_substitution = did_google_substitution or google
    ...
    for element in chromium_grd.iter():
        ...
        if element.tag == 'part':
            ...
            assert grdp_file in grdp_files, \
                f'GRDP file "{grdp_file}" must be added to //rebel/BUILD.gn'
