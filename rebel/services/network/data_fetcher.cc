...
void DataFetcher<T>::OnFailure(const std::string& error_message) {
  if (configuration_.notify_on_failure_) {
    ...
    switch (configuration_.response_type_) {
      case ResponseType::JSON:
        OnFetchComplete(base::Value(base::Value::Type::DICTIONARY), &error);
        break;
      case ResponseType::Raw:
...
void RegisterBrowserProcessDataFetcherProfilePrefs(PrefRegistrySimple* prefs) {
  RemoteNtpApiAllowList::RegisterProfilePrefs(prefs);
}

void RegisterNetworkServiceDataFetcherProfilePrefs(PrefRegistrySimple* prefs) {}
