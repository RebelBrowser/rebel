...
// Register the profile preferences used by each implementation of this class.
// This is just a proxy to call into the implementations so that newly added
// implementations can just append to this function, rather than having to find
// all of the upstream files that would need to be modified.
void COMPONENT_EXPORT(NETWORK_CPP)
    RegisterBrowserProcessDataFetcherProfilePrefs(PrefRegistrySimple* prefs);
void COMPONENT_EXPORT(NETWORK_CPP)
    RegisterNetworkServiceDataFetcherProfilePrefs(PrefRegistrySimple* prefs);
...
// should also: add themselves to one of the pref registration methods above and
// to the list of explicit instantiations at the bottom of |data_fetcher.cc|.
template <typename T>
