...
void RemoteNtpThemeProvider::StoreBackgroundImage(
  ...
  if (collection_id.empty()) {
    ...
    base::Value::Dict new_background =
        pref_service_->GetDict(kRemoteNtpBackgroundDict).Clone();
...
void RemoteNtpThemeProvider::ApplyColorToTheme(
    content::WebContents* tab,
    rebel::mojom::RemoteNtpTheme* theme) {
  if (!theme->using_generated_theme) {
    return;
  }

  const auto& color_provider = tab->GetColorProvider();
  theme->color_dark =
      color_provider.GetColor(kColorTabBackgroundActiveFrameActive);
  theme->color_light = color_provider.GetColor(kColorNewTabPageBackground);
}
...
rebel::mojom::RemoteNtpThemePtr RemoteNtpThemeProvider::CreateTheme() {
  ...
  if (theme_service && theme_service->UsingExtensionTheme()) {
    SetExtensionThemeDetails(theme_service->GetThemeID(), theme.get());
    ...
    theme->using_generated_theme = true;
  }

  const base::Value::Dict& background =
      pref_service_->GetDict(kRemoteNtpBackgroundDict);
