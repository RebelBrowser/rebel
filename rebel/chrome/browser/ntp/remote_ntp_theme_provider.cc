...
rebel::mojom::RemoteNtpThemePtr RemoteNtpThemeProvider::CreateTheme() {
  ...
  bool using_extension_theme = false;

  if (theme_service_) {
    if (theme_service_->UsingExtensionTheme()) {
      SetExtensionThemeDetails(theme_service_->GetThemeID(), theme.get());
      using_extension_theme = true;
    } else if (theme_service_->UsingAutogeneratedTheme()) {
      const SkColor color = theme_service_->GetAutogeneratedThemeColor();
  ...
  if (custom_background_service_ && !using_extension_theme) {
    if (auto background = custom_background_service_->GetCustomBackground()) {
      theme->collection_id = background->collection_id;
