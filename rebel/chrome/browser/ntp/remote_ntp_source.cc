...
void ServeRawImage(content::URLDataSource::GotDataCallback callback,
  ...
  auto data = base::RefCountedString::TakeString(&image_data);
...
void RemoteNtpSource::StartDataRequest(
  ...
  const std::string file_path = url.path().substr(1);
...
std::string RemoteNtpSource::GetMimeType(const std::string& file_path) {
