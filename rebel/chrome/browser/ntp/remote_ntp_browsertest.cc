...
class TestRemoteNtpServiceObserver : public rebel::RemoteNtpService::Observer {
 public:
  ...
  void WaitForThemeColor(int expected_theme_color_id) {
    DCHECK(!quit_closure_theme_);

    if (theme_color_id_ == expected_theme_color_id) {
      return;
    }

    expected_theme_color_id_ = expected_theme_color_id;
 ...
 private:
  ...
  rebel::RemoteNtpService* const remote_ntp_service_;
  ...
  int theme_color_id_{-1};
  int expected_theme_color_id_{-1};
 ...
 protected:
  void ValidateBackground(const content::ToRenderFrameHost& adapter,
    ...
    EXPECT_EQ(collection_id,
              content::EvalJs(adapter, "ntpBackground.collectionId"));
  ...
  void ValidateColor(const content::ToRenderFrameHost& adapter,
                     const char* color) {
    auto is_valid_intensity = [](content::EvalJsResult result) {
      const int color = result.ExtractInt();
      return (color >= 0) && (color <= 255);
    };

    EXPECT_EQ(4,
              content::EvalJs(adapter, base::StringPrintf("%s.length", color)));
    EXPECT_PRED1(is_valid_intensity,
                 content::EvalJs(adapter, base::StringPrintf("%s[0]", color)));
    EXPECT_PRED1(is_valid_intensity,
                 content::EvalJs(adapter, base::StringPrintf("%s[1]", color)));
    EXPECT_PRED1(is_valid_intensity,
                 content::EvalJs(adapter, base::StringPrintf("%s[2]", color)));
    EXPECT_PRED1(is_valid_intensity,
                 content::EvalJs(adapter, base::StringPrintf("%s[3]", color)));
  }

  void ValidateColor(const content::ToRenderFrameHost& adapter,
                     const char* color,
                     SkColor expected_color) {
    ValidateColor(adapter, color);

    EXPECT_EQ(static_cast<int>(SkColorGetR(expected_color)),
              content::EvalJs(adapter, base::StringPrintf("%s[0]", color)));
    EXPECT_EQ(static_cast<int>(SkColorGetG(expected_color)),
              content::EvalJs(adapter, base::StringPrintf("%s[1]", color)));
    EXPECT_EQ(static_cast<int>(SkColorGetB(expected_color)),
              content::EvalJs(adapter, base::StringPrintf("%s[2]", color)));
    EXPECT_EQ(static_cast<int>(SkColorGetA(expected_color)),
              content::EvalJs(adapter, base::StringPrintf("%s[3]", color)));
  }
