...
class InternalCodeSignConfig(ChromiumCodeSignConfig):
    ...
    def override_parts_for_rebel(self, parts):
        ...
        # We currently do not brand the privileged-helper binary, as we do not
        # use it, in favor of Sparkle. The signing scripts assume the binary is
        # branded, though. So reset it to the unbranded version here.
        privileged_helper = f'{self.base_bundle_id}.UpdaterPrivilegedHelper'
        privileged_helper = re.compile(privileged_helper)

        parts['privileged-helper'].path = privileged_helper.sub(
            'org.chromium.Chromium.UpdaterPrivilegedHelper',
            parts['privileged-helper'].path)
        parts['privileged-helper'].identifier = privileged_helper.sub(
            'org.chromium.Chromium.UpdaterPrivilegedHelper',
